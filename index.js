...
const manifest = {
  ...
  "logo": "https://raw.githubusercontent.com/mik25/stremio-greek-tv/master/open.png",
  "resources": ["stream"],
  "types": ["movie", "series"],
  "idPrefixes": ["tt"],
  "catalogs": []
...
serveHTTP(addonInterface, { port: process.env.PORT || 7777, hostname: '0.0.0.0' }).then(() => {
  console.log(`Addon running at: http://0.0.0.0:${process.env.PORT || 7777}/stremioget/stremio/v1`);
  console.log(`HTTP addon accessible at: http://127.0.0.1:${process.env.PORT || 7777}/manifest.json`);
}).catch(err => {
  console.error(err);
});